...
- **技能分立分配**：檢核項改由共享技能庫驅動（`lab_skills`／`lab_skill_modules`，單份 JSON；雲端無資料時使用 `index.html` 內建 `SKILL_SEED`，內容定稿見 `SKILLS.md`）。首次在管理介面修改時才寫入雲端。
- **四種分配途徑**：階段技能（依年資，`tier`）＋領域過濾（`track`）；模組技能（`module`，掛在專案的 `modules` 欄位上，參與者自動獲得）；教授指定（`lab_pi_checks`）；學生自訂（`lab_custom_checks`）。
- **必備與升級**：`core:true` 的階段技能為升級門檻——總達成 ≥80% 且必備全數完成；同一學制內上一階段未完成的必備自動帶入（carry-over）。模組必備不擋升級。
- **管理介面**：全實驗室總覽 →「技能庫管理」：新增／編輯／停用（off）／刪除技能、新增模組，全部免改程式碼。
- **主項編輯**：任何檢核項可按 ✎ 編輯；內建項為每學生覆蓋（`lab_check_edits`），可還原預設。
- 歷程備註：`28faa7f` 曾誤用舊底版蓋掉雙語、匯出等功能，已於 `5266638` 以 `a4d7d26` 為基底復原並回移植教授指定檢核。
